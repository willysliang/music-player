 ...
 * @ Modified time: 2023-01-06 14:57:53
 * @ Description: ParticleAnimation 粒子动画倒计时
 -->
...
const dotsRef = ref<HTMLCanvasElement | null>(null)
const numberRef = ref<HTMLCanvasElement | null>(null)
/** 初始化canvas和数字文本 */
const init = () => {
  if (
    numberRef.value instanceof HTMLCanvasElement &&
    dotsRef.value instanceof HTMLCanvasElement
  ) {
    // 初始化canvas-number
...
const tweenDots = (dot, pos, type) => {
  if (type === 'space') {
    gsap.to(dot, (3 + Math.round(Math.random() + 100) / 100), {
    ...
    gsap.to(dot, (1.5 + Math.round(Math.random() * 100) / 100), {
...
const breakNumber = () => {
  canvas.numberPixelCoordinates.forEach((item, i) => {
...
const drawNumber = (num) => {
  canvas.numberStageCtx = numberRef.value?.getContext('2d')
  canvas.numberStageCtx.clearRect(0, 0, canvas.numberStageWidth, canvas.numberStageHeight)
  canvas.numberStageCtx.fillStyle = '#24282f'
  canvas.numberStageCtx.textAlign = 'center';
  canvas.numberStageCtx.font = 'bolid 118px Lato'
  canvas.numberStageCtx.fillText(num, 250, 300)

  if (numberRef.value instanceof HTMLCanvasElement) {
    const ctx = numberRef.value.getContext('2d')
    const imageData = ctx?.getImageData(0, 0, canvas.numberStageWidth, canvas.numberStageHeight).data ?? []
    ...
    for (let i = imageData.length; i > 0; i -= 4) {
      if (imageData[i] !== 0) {
        ...
        if ((x && x % (canvas.circleRadius * 2 + 3) === 0) && (y && y % (canvas.circleRadius * 2 + 3) === 0)) {
